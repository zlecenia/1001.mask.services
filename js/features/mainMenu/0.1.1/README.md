...
Główne menu aplikacji z kontrolą dostępu opartą na rolach (OPERATOR, ADMIN, SUPERUSER, SERWISANT)
...
node tools/dev/componentDevServer.js js/features/mainMenu/0.1.0 3001
```
...
const result = await mainMenu.init(context);

// Use component
if (mainMenu.component) {
  // Vue component available
  const VueComponent = mainMenu.component;
}

// Handle actions
const response = mainMenu.handle({
  action: 'render',
  data: { /* your data */ }
});
```
...
│   ├── mainMenu.js              # Vue component ✅
│   ├── mainMenu.test.js         # Component tests ✅
...
- Key properties: `layout`, `menuItem`, `accessibility`

#### `roleBasedAccess`
RoleBasedAccess configuration
- Key properties: `OPERATOR`, `ADMIN`, `SUPERUSER`, ...

#### `menuStructure`
MenuStructure configuration
- Key properties: `dashboard`, `monitoring`, `tests`, ...

#### `data`
Default runtime data values
- Key properties: `currentUser`, `currentRoute`, `menuState`
...
- Key properties: `enableCaching`, `lazyLoading`, `virtualScrolling`

#### `security`
Security and validation rules
- Key properties: `validatePermissions`, `auditAccess`, `sessionTimeout`
...
```bash
# Run component tests
npm test -- mainMenu.test.js

# Validate configuration
npm run module:validate mainMenu
...
4. Test changes with `npm run component:dev:mainMenu`
...
**Generated**: 2025-09-27T07:16:41.994Z  
**Generator**: componentReadmeGenerator v1.0.0  
**Component**: mainMenu@0.1.0
